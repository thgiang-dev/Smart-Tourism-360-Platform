    ...
    <div v-else-if="detail" class="space-y-10 animate-fade-in">
      ...
      <div class="relative w-full h-[380px] md:h-[520px] bg-slate-900 overflow-hidden shadow-2xl">
        ...
        <div class="absolute top-6 left-4 md:left-8 z-10">
          <button 
            @click="goBack"
            class="premium-btn inline-flex items-center space-x-2 px-5 py-2.5 bg-slate-950/60 hover:bg-slate-900/90 text-white font-bold rounded-2xl text-xs border border-white/10 hover:border-white/20 shadow-xl backdrop-blur-md transition duration-200 focus:outline-none"
          >
            <ArrowLeftIcon class="w-4 h-4 text-teal-400" />
            <span>Quay lại</span>
          </button>
        </div>
