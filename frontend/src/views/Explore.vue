<template>
  <div class="h-screen flex flex-col overflow-hidden bg-slate-50 font-sans">
    ...
    <div class="flex-grow flex flex-col md:flex-row overflow-hidden relative">
      ...
      <aside 
        :class="[
          'bg-white border-r border-slate-200 flex flex-col z-20 flex-shrink-0 shadow-sm relative transition-all duration-300 ease-in-out',
      ...
      >
        ...
        <button 
          ...
          :class="[
            'absolute top-1/2 -translate-y-1/2 z-30 w-6 h-12 flex items-center justify-center border rounded-r-xl shadow-md transition-all duration-200 pointer-events-auto focus:outline-none',
            isSidebarOpen 
              ? 'left-full -translate-x-1/2 bg-white hover:bg-slate-50 border-slate-200 text-slate-500 hover:text-teal-600' 
      ...
      <main class="flex-grow h-full w-full z-10 relative">
        ...
        <router-link 
          to="/explore-3d" 
          class="premium-btn absolute top-5 right-5 z-20 px-4 py-2.5 bg-white border border-slate-200 rounded-2xl text-xs font-black text-slate-700 shadow-lg hover:bg-slate-50 flex items-center space-x-1.5 transition"
        >
          <GlobeIcon class="w-4 h-4 text-teal-600 animate-pulse" />
          <span>Bản đồ 3D</span>
        </router-link>
  ...
  try {
    ...
    if (response.success) {
      ...
      if (searchKeyword.value) {
        trackEvent({
          eventName: 'search_destination',
          targetType: 'search',
          metadata: { keyword: searchKeyword.value, resultCount: response.data.length }
        })
      }
      if (selectedCategory.value) {
        ...
        trackEvent({
          ...
          metadata: { categoryName: cat ? cat.name : '', resultCount: response.data.length }
        })
      }
    }
  } catch (err) {
  ...
  map.on('popupopen', (e) => {
    ...
    links.forEach(link => {
      link.addEventListener('click', (event) => {
        ...
        const clickType = link.getAttribute('data-type')
        if (url) {
          if (destId) {
            if (clickType === 'tour') {
              trackEvent({
              ...
              trackEvent({
                eventName: 'open_destination_from_map',
                targetType: 'destination',
                targetId: destId
              })
            }
...
const updateMapMarkers = () => {
  ...
  destinations.value.forEach(dest => {
    ...
    const popupContent = `
      <div class="p-2 max-w-[220px] font-sans space-y-3">
        ...
        <div class="flex gap-2 pt-2 border-t border-slate-100 w-full">
          <a href="/destinations/${dest.slug || dest.id}" data-id="${dest.id}" data-type="detail" class="flex-1 py-1.5 bg-slate-50 hover:bg-slate-100 text-slate-700 text-[10px] font-black text-center rounded-lg border border-slate-200/60 transition duration-150">Chi tiết</a>
          ${dest.hasVirtualTour ? `<a href="/destinations/${dest.id}/tour" data-id="${dest.id}" data-type="tour" class="flex-1 py-1.5 bg-teal-600 hover:bg-teal-500 text-white text-[10px] font-black text-center rounded-lg shadow-sm transition duration-150 flex items-center justify-center space-x-0.5"><span>Vào 360°</span></a>` : ''}
        </div>
      </div>
    `
    ...
    marker.on('click', () => {
      trackEvent({
        eventName: 'click_map_marker',
        targetType: 'destination',
        targetId: dest.id,
        metadata: { source: 'explore_map' }
      })
    })
