<template>
  <div class="flex flex-col lg:flex-row gap-6 min-h-[calc(100vh-140px)]">
    ...
    <div class="flex-grow lg:w-3/4 flex flex-col space-y-4">
      <!-- Top controls bar -->
      <div class="flex items-center justify-between bg-white p-4 rounded-2xl border border-slate-200/60 shadow-sm">
        <div class="flex items-center space-x-3">
          <button 
            @click="goBack" 
            class="p-2 hover:bg-slate-50 border border-slate-200 rounded-xl text-slate-600 transition"
            title="Quay lại danh sách cảnh"
          >
            <ChevronLeftIcon class="w-4 h-4" />
          </button>
          <div>
            <h3 class="font-bold text-slate-800 text-sm">Góc cảnh: {{ panorama?.title || 'Đang tải...' }}</h3>
            <p class="text-[10px] text-slate-400">Click trực tiếp vào ảnh 360° để chọn tọa độ đặt Hotspot mới.</p>
          </div>
        </div>

        <!-- Mode Toggle -->
        <div class="flex items-center bg-slate-100 rounded-xl p-1 border border-slate-200/40">
          <button 
            @click="activeMode = 'edit'"
            :class="[
              'px-3 py-1.5 rounded-lg text-xs font-bold transition flex items-center space-x-1.5',
              activeMode === 'edit' ? 'bg-teal-500 text-slate-950 shadow-sm' : 'text-slate-500 hover:text-slate-800'
            ]"
          >
          ...
          <button 
            ...
            :class="[
              'px-3 py-1.5 rounded-lg text-xs font-bold transition flex items-center space-x-1.5',
              activeMode === 'preview' ? 'bg-teal-500 text-slate-950 shadow-sm' : 'text-slate-500 hover:text-slate-800'
            ]"
          >
      ...
      <!-- 360 Canvas -->
      <div class="relative w-full h-[550px] bg-slate-950 rounded-2xl overflow-hidden border border-slate-800 shadow-inner group">
        ...
        <div class="absolute top-4 left-4 pointer-events-none z-10 select-none">
          <span 
            class="inline-flex items-center px-3 py-1.5 rounded-xl text-xs font-bold uppercase tracking-wider backdrop-blur-md shadow-md border border-white/10"
        ...
        <div v-if="activeMode === 'edit'" class="absolute inset-0 pointer-events-none flex items-center justify-center opacity-70">
          <div class="w-10 h-10 border-2 border-dashed border-teal-400 rounded-full flex items-center justify-center animate-pulse">
    ...
    <div class="lg:w-1/4 flex flex-col space-y-6">
      ...
      <div v-if="!showFormModal" class="bg-white p-6 rounded-2xl border border-slate-200/60 shadow-sm flex flex-col h-full space-y-4">
        <div class="flex justify-between items-center border-b border-slate-100 pb-3">
          <h4 class="text-sm font-bold text-slate-800 uppercase tracking-wider">Danh sách Hotspot</h4>
          <span class="px-2 py-0.5 bg-slate-150 text-slate-600 rounded-full text-[10px] font-bold">
            {{ hotspots.length }} điểm
          </span>
        </div>

        <!-- Sidebar Content -->
        <div class="flex-grow overflow-y-auto max-h-[400px] space-y-2 pr-1">
          <div v-if="hotspots.length === 0" class="text-center py-8 text-slate-400 space-y-1">
            <CompassIcon class="w-8 h-8 mx-auto opacity-40" />
            <p class="text-xs">Chưa có điểm tương tác nào.</p>
          </div>

          <div 
            v-for="h in hotspots" 
            :key="h.id"
            class="flex items-center justify-between p-3 rounded-xl border border-slate-100 hover:border-slate-200/80 hover:bg-slate-50/50 transition duration-150 group"
          >
            <div class="flex items-center space-x-2.5 overflow-hidden">
              ...
              <div class="overflow-hidden">
                <span class="text-xs font-bold text-slate-700 block truncate">{{ h.title }}</span>
                <span class="text-[9px] text-slate-400 block font-mono capitalize">{{ h.type }}</span>
              </div>
            </div>
            ...
            <div class="flex items-center space-x-0.5 opacity-0 group-hover:opacity-100 transition duration-150">
              <button 
                ...
                class="p-1 hover:bg-slate-200 rounded text-slate-500 hover:text-slate-800 transition"
                title="Xem vị trí"
              >
              ...
              <button 
                ...
                class="p-1 hover:bg-slate-200 rounded text-slate-500 hover:text-teal-600 transition"
                title="Chỉnh sửa"
              >
                <EditIcon class="w-3.5 h-3.5" />
              </button>
              <button 
                ...
                class="p-1 hover:bg-slate-200 rounded text-slate-500 hover:text-red-600 transition"
                title="Xóa"
              >
        ...
        <button 
          ...
          class="w-full py-2.5 bg-slate-900 hover:bg-slate-800 text-white font-semibold text-xs rounded-xl transition duration-150 flex items-center justify-center space-x-1"
        >
      ...
      <div v-else class="bg-white p-5 rounded-2xl border border-slate-200/60 shadow-sm flex flex-col h-full space-y-4 overflow-y-auto max-h-[calc(100vh-140px)]">
        <div class="flex justify-between items-center border-b border-slate-100 pb-3">
          <div>
            <h4 class="text-xs font-bold text-slate-800 uppercase tracking-wider">
              {{ isEditMode ? 'Sửa Hotspot' : 'Thêm Hotspot' }}
            </h4>
            <p class="text-[9px] text-slate-400">Điều chỉnh thông tin và vị trí.</p>
          </div>
          <button 
            type="button"
        ...
        <form @submit.prevent="submitForm" class="space-y-4 pt-1">
          <!-- Type selection -->
          <div>
            <label class="block text-[10px] font-semibold text-slate-500 uppercase tracking-wider mb-1">Loại điểm tương tác *</label>
            <select 
              v-model="modalForm.type"
              required
              class="w-full px-2.5 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs focus:outline-none focus:border-teal-500/80 focus:bg-white transition"
            >
          ...
          <!-- Title -->
          <div>
            <label class="block text-[10px] font-semibold text-slate-500 uppercase tracking-wider mb-1">Tiêu đề điểm *</label>
            <input 
              v-model="modalForm.title"
              type="text"
              required
              placeholder="Ví dụ: Đi tới Chính điện..."
              class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs focus:outline-none focus:border-teal-500/80 focus:bg-white transition"
            />
          </div>

          <!-- Description -->
          <div>
            <label class="block text-[10px] font-semibold text-slate-500 uppercase tracking-wider mb-1">Mô tả chi tiết</label>
            <textarea 
              v-model="modalForm.description"
              rows="2"
              placeholder="Mô tả bổ sung..."
              class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs focus:outline-none focus:border-teal-500/80 focus:bg-white transition resize-none"
            ></textarea>
          </div>

          <!-- Navigation Type: Target Panorama selection -->
          <div v-if="modalForm.type === 'navigation'">
            <label class="block text-[10px] font-semibold text-slate-500 uppercase tracking-wider mb-1">Cảnh chuyển đến *</label>
            <select 
          ...
          <div v-if="['audio', 'video', 'image', 'model3d'].includes(modalForm.type)">
            <label class="block text-[10px] font-semibold text-slate-500 uppercase tracking-wider mb-1">Tập tin đa phương tiện *</label>
            <div class="flex items-center space-x-1.5">
              ...
              <button 
                ...
                class="px-3 py-2 bg-slate-900 hover:bg-slate-800 text-white rounded-xl text-[10px] font-semibold transition"
              >
                Chọn
              </button>
            </div>
          ...
          <div v-if="modalForm.type === 'link'">
            <label class="block text-[10px] font-semibold text-slate-500 uppercase tracking-wider mb-1">Đường dẫn ngoài *</label>
            <input 
              v-model="modalForm.externalUrl"
              type="url"
              required
              placeholder="https://..."
              class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-[10px] focus:outline-none focus:border-teal-500/80 focus:bg-white transition font-mono"
            />
          </div>
          ...
          <div class="grid grid-cols-2 gap-2">
            <div>
              <label class="block text-[9px] font-semibold text-slate-500 uppercase tracking-wider mb-1">Yaw (Radian)</label>
              <input 
                :value="parseFloat(modalForm.yaw).toFixed(4)"
                type="text"
                readonly
                class="w-full px-2 py-1.5 bg-slate-100 border border-slate-200 rounded-xl text-[10px] focus:outline-none font-mono"
              />
            </div>
            <div>
              <label class="block text-[9px] font-semibold text-slate-500 uppercase tracking-wider mb-1">Pitch (Radian)</label>
              <input 
          ...
          <div class="flex items-center justify-between text-[9px] text-slate-400 bg-slate-50 p-2.5 rounded-xl border border-slate-200/40">
            <span>Click trên ảnh để đổi vị trí.</span>
            <button 
              type="button" 
              @click="updateFormCoordsFromCenter" 
              class="text-teal-600 hover:text-teal-700 font-bold transition flex items-center space-x-1"
            >
          ...
          <div v-if="modalError" class="text-[10px] font-semibold text-red-600 bg-red-50 p-2 rounded-lg border border-red-100">
            {{ modalError }}
          </div>
          ...
          <div class="flex items-center space-x-2 pt-3 border-t border-slate-100">
            <button 
              type="submit" 
              class="flex-grow py-2 bg-teal-500 hover:bg-teal-600 text-slate-950 font-bold rounded-xl shadow-lg hover:shadow-teal-500/10 active:scale-[0.98] transition flex items-center justify-center space-x-1.5 disabled:opacity-50 text-xs"
              :disabled="submitting"
            >
            ...
            <button 
              type="button"
              @click="closeFormModal"
              class="px-3 py-2 border border-slate-200 hover:bg-slate-50 text-slate-600 font-semibold rounded-xl text-xs transition"
              :disabled="submitting"
            >
              Hủy
            </button>
    ...
    >
      <div class="bg-white rounded-2xl w-full max-w-2xl shadow-2xl border border-slate-100 overflow-hidden flex flex-col max-h-[80vh]">
        <div class="p-6 border-b border-slate-100 flex justify-between items-center">
          <div class="space-y-1">
            <h3 class="text-base font-bold text-slate-800">Chọn tệp từ thư viện</h3>
        ...
        <div class="p-6 overflow-y-auto flex-1 min-h-[200px]">
          ...
          <div v-else-if="selectorMedia.length === 0" class="text-center py-12 space-y-3">
            ...
            <p class="text-sm font-medium text-slate-500">Thư viện trống hoặc không có tệp phù hợp.</p>
          </div>
          ...
          <div v-else class="grid grid-cols-3 sm:grid-cols-4 gap-4">
            ...
            >
              ...
              <div v-else class="space-y-2 text-slate-400">
                <MusicIcon v-if="img.mediaType === 'audio'" class="w-8 h-8 mx-auto text-fuchsia-400" />
                <VideoIcon v-else-if="img.mediaType === 'video'" class="w-8 h-8 mx-auto text-red-400" />
                <CompassIcon v-else class="w-8 h-8 mx-auto text-violet-400" />
                <span class="text-[10px] block truncate max-w-[100px] font-semibold text-slate-600">{{ img.fileName }}</span>
              </div>
    ...
    >
      <div class="bg-slate-900 border border-slate-800 text-white rounded-2xl w-full max-w-lg shadow-2xl overflow-hidden flex flex-col">
        <!-- Preview Header -->
        <div class="p-5 border-b border-slate-800 flex justify-between items-center bg-slate-950/40">
          ...
          <button @click="closePreview" class="p-1 hover:bg-slate-800 rounded-lg text-slate-400 hover:text-white transition">
            <XIcon class="w-5 h-5" />
          </button>
        </div>
        ...
        <div class="p-6 flex flex-col items-center justify-center space-y-4">
          ...
          <p v-if="previewingHotspot.description" class="text-xs text-slate-300 w-full text-center">{{ previewingHotspot.description }}</p>

          <!-- Type specific preview views -->

          <!-- Image Preview -->
          <div v-if="previewingHotspot.type === 'image' && previewingHotspot.mediaUrl" class="w-full aspect-[4/3] rounded-xl overflow-hidden bg-black flex items-center justify-center">
            <img :src="previewingHotspot.mediaUrl" class="w-full h-full object-contain" />
          </div>

          <!-- Video Preview -->
          <div v-else-if="previewingHotspot.type === 'video' && previewingHotspot.mediaUrl" class="w-full aspect-video rounded-xl overflow-hidden bg-black flex items-center justify-center">
            <video :src="previewingHotspot.mediaUrl" controls autoplay class="w-full h-full"></video>
          </div>

          <!-- Audio Preview -->
          <div v-else-if="previewingHotspot.type === 'audio' && previewingHotspot.mediaUrl" class="w-full p-4 bg-slate-950 border border-slate-800 rounded-xl flex items-center space-x-4">
            <button 
              @click="toggleAudioPlay" 
              class="w-12 h-12 bg-teal-500 text-slate-950 rounded-full flex items-center justify-center hover:scale-105 active:scale-95 transition"
            >
              <PauseIcon v-if="audioPlaying" class="w-5 h-5 fill-slate-950" />
              <PlayIcon v-else class="w-5 h-5 fill-slate-950 ml-0.5" />
            </button>
            <div class="flex-grow">
              <span class="text-xs font-semibold text-slate-300 block truncate">{{ getFileNameFromUrl(previewingHotspot.mediaUrl) }}</span>
              <span class="text-[10px] text-slate-500 font-mono">Trình phát Audio thô</span>
            </div>
            <audio ref="audioPlayer" :src="previewingHotspot.mediaUrl" class="hidden" @ended="audioPlaying = false"></audio>
          </div>

          <!-- Model 3D Placeholder -->
          <div v-else-if="previewingHotspot.type === 'model3d'" class="text-center py-6 space-y-2 text-slate-500">
            <CompassIcon class="w-12 h-12 mx-auto animate-spin text-teal-400" />
            <p class="text-xs font-semibold">Khởi chạy Mô hình 3D (Sprint 10+)</p>
          </div>

          <!-- Info Fallback -->
          <div v-else-if="previewingHotspot.type === 'info'" class="py-6 text-center text-slate-400">
            <span class="text-xs">Không có xem trước đa phương tiện cho nội dung văn bản.</span>
          </div>
        </div>
      </div>
    </div>
