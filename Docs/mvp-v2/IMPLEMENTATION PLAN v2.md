...
| Sprint 13 | Route / Itinerary | Tuyến tham quan | Admin tạo tuyến, du khách xem tuyến trên bản đồ | [x] Đã hoàn thành |
...
\- \[x\] Thêm ảnh/video nền hoặc ảnh du lịch nổi bật.  
\- \[x\] Thêm CTA chính “Khám phá bản đồ”.  
\- \[x\] Thêm CTA phụ “Trải nghiệm tour 360”.  
\- \[x\] Thêm section Featured Categories.  
\- \[x\] Thêm section Featured Destinations.  
\- \[x\] Thêm section 360 Experience.  
\- \[x\] Thêm animation fade/slide nhẹ.  
...
\- \[x\] Cải thiện layout desktop.  
\- \[x\] Cải thiện layout mobile.  
\- \[x\] Tạo search/filter bar trực quan hơn.  
\- \[x\] Category filter dạng chip.  
\- \[x\] Marker popup đẹp hơn.  
\- \[x\] Hiển thị badge “Có tour 360”.  
\- \[x\] Thêm empty state khi không có kết quả.  
\- \[x\] Thêm loading state khi tải marker.  
\- \[x\] Thêm nút chuyển sang 3D Map placeholder nếu muốn chuẩn bị Sprint 16\.

---

## **6.5. Checklist Destination List & Card**

\- \[x\] Tạo DestinationCard v2.  
\- \[x\] Card có cover image.  
\- \[x\] Card có category badge.  
\- \[x\] Card có tour 360 badge.  
\- \[x\] Card có mô tả ngắn.  
\- \[x\] Card có CTA “Xem chi tiết”.  
\- \[x\] Hover effect cho desktop.  
\- \[x\] Grid responsive: desktop 3 cột, tablet 2 cột, mobile 1 cột.
...
\- \[x\] Overlay gradient.  
\- \[x\] Category badge.  
\- \[x\] CTA “Tham quan 360” nổi bật.  
\- \[x\] Destination info panel.  
\- \[x\] Gallery ảnh.  
\- \[x\] Audio/video section nếu có.  
\- \[x\] Bản đồ vị trí nhỏ.  
\- \[x\] Nearby destinations nếu có thể.  
\- \[x\] Responsive mobile.
...
\- \[x\] Tạo Tour360TopBar.  
\- \[x\] Tạo Tour360ControlBar.  
\- \[x\] Tạo SceneListPanel.  
\- \[x\] Scene list có thumbnail/title.  
\- \[x\] Thêm first-time guide overlay.  
\- \[x\] Thêm hướng dẫn: kéo để xoay, bấm mũi tên để di chuyển.  
\- \[x\] Cải thiện hotspot icon.  
\- \[x\] Hotspot navigation có hiệu ứng pulse nhẹ.  
\- \[x\] Hotspot label khi hover/tap.  
\- \[x\] Transition mượt khi chuyển panorama.  
\- \[x\] Mobile bottom sheet cho hotspot info/audio.  
\- \[x\] Loading overlay khi đổi panorama.

---

## **6.8. Checklist Admin Dashboard**

\- \[x\] Cải thiện admin stat cards.  
\- \[x\] Cải thiện destination table.  
\- \[x\] Cải thiện status/category/media badges.  
\- \[x\] Form tạo/sửa địa điểm chia section rõ ràng.  
\- \[x\] Cải thiện Media Library card.  
\- \[x\] Cải thiện Panorama card.  
\- \[x\] Cải thiện Hotspot Editor side panel.  
\- \[x\] Giữ nguyên logic CRUD hiện có.

---

## **6.9. Checklist Test Sprint 12**

\- \[x\] Public home page không lỗi.  
\- \[x\] Explore map load marker đúng.  
\- \[x\] Destination detail mở đúng.  
\- \[x\] Tour 360 load đúng.  
\- \[x\] Hotspot navigation vẫn chuyển panorama.  
\- \[x\] Admin login vẫn hoạt động.  
\- \[x\] Admin tạo/sửa destination vẫn hoạt động.  
\- \[x\] Upload media vẫn hoạt động.  
\- \[x\] npm run build thành công.  
\- \[x\] docker compose up \-d \--build thành công.
...
\- \[x\] Tạo entity Route.  
\- \[x\] Tạo entity RouteDestination.  
\- \[x\] Tạo bảng routes.  
\- \[x\] Tạo bảng route\_destinations.  
\- \[x\] Thêm quan hệ routes.region\_id.  
\- \[x\] Thêm quan hệ route\_destinations.route\_id.  
\- \[x\] Thêm quan hệ route\_destinations.destination\_id.  
\- \[x\] Thêm index region\_id.  
\- \[x\] Thêm unique index region\_id \+ slug.  
\- \[x\] Thêm unique index route\_id \+ destination\_id.  
\- \[x\] Tạo migration.  
\- \[x\] Seed 2-3 route mẫu.
...
\- \[x\] Tạo Route DTOs.  
\- \[x\] Tạo RouteDestination DTOs.  
\- \[x\] Tạo RouteService.  
\- \[x\] Tạo public RoutesController.  
\- \[x\] Tạo AdminRoutesController.  
\- \[x\] GET /api/routes.  
\- \[x\] GET /api/routes/{id}.  
\- \[x\] GET /api/admin/routes.  
\- \[x\] POST /api/admin/routes.  
\- \[x\] PUT /api/admin/routes/{id}.  
\- \[x\] PATCH /api/admin/routes/{id}/status.  
\- \[x\] DELETE /api/admin/routes/{id}.  
\- \[x\] POST /api/admin/routes/{routeId}/destinations.  
\- \[x\] POST /api/admin/routes/{routeId}/destinations/bulk.  
\- \[x\] PATCH /api/admin/routes/{routeId}/destinations/reorder.  
\- \[x\] DELETE /api/admin/routes/{routeId}/destinations/{routeDestinationId}.  
\- \[x\] Validate slug không trùng trong cùng region.  
\- \[x\] Validate route phải có ít nhất 2 destinations trước khi publish.
...
\- \[x\] Tạo /admin/routes.  
\- \[x\] Tạo route table.  
\- \[x\] Tạo filter theo status/theme/keyword.  
\- \[x\] Tạo /admin/routes/create.  
\- \[x\] Tạo /admin/routes/:id/edit.  
\- \[x\] Tạo route form.  
\- \[x\] Tạo /admin/routes/:id/destinations.  
\- \[x\] Tạo destination picker.  
\- \[x\] Tạo route destination list.  
\- \[x\] Cho phép thêm destination vào route.  
\- \[x\] Cho phép remove destination khỏi route.  
\- \[x\] Cho phép reorder bằng nút up/down.  
\- \[x\] Cho phép sửa estimatedStayTime/note.  
\- \[x\] Tạo preview route map.
...
\- \[x\] Tạo /routes.  
\- \[x\] Tạo route list page.  
\- \[x\] Tạo RouteCard.  
\- \[x\] Tạo /routes/:id hoặc /routes/:slug.  
\- \[x\] Tạo route detail page.  
\- \[x\] Tạo RouteMap bằng Leaflet.  
\- \[x\] Hiển thị marker đánh số.  
\- \[x\] Vẽ polyline nối điểm theo displayOrder.  
\- \[x\] Tạo itinerary timeline.  
\- \[x\] Mỗi item có nút xem destination detail.  
\- \[x\] Mỗi item có nút tour 360 nếu hasVirtualTour.  
...
\- \[x\] Admin tạo route mới.  
\- \[x\] Admin thêm nhiều địa điểm vào route.  
\- \[x\] Admin reorder địa điểm.  
\- \[x\] Admin publish route.  
\- \[x\] Public route list chỉ hiển thị route published.  
\- \[x\] Public route detail hiển thị đúng thứ tự.  
\- \[x\] Route map hiển thị marker đúng tọa độ.  
\- \[x\] Polyline nối đúng thứ tự.  
\- \[x\] Click destination mở trang chi tiết.  
\- \[x\] Click tour 360 mở tour.  
\- \[x\] Không làm hỏng destination/tour cũ.
